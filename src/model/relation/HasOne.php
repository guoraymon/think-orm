...
class HasOne extends OneToOne
{
    ...
    public function getRelationCountQuery(Closure $closure = null, $aggregate = 'count', $field = '*', &$name = null)
    {
        ...
        return $this->query->whereExp($this->foreignKey, '=' . $this->parent->getTable() . '.' . $this->localKey)->fetchSql()->{$aggregate}($field);
    }
    /**
     * 关联统计
     * @access public
    ...
    public function hasWhere($where = [], $fields = null, $joinType = '')
    {
        ...
        return $this->parent->db()->alias($model)->field($fields)->join([$table => $relation], $model . '.' . $this->localKey . '=' . $relation . '.' . $this->foreignKey, $joinType ?: $this->joinType)->where($where);
    }
    /**
     * 预载入关联查询（数据集）
     * @access protected
    ...
    protected function eagerlySet(array &$resultSet, $relation, array $subRelation = [], Closure $closure = null, array $cache = [])
    {
        ...
        if (!empty($range)) {
            ...
            $data = $this->eagerlyWhere([[$foreignKey, 'in', $range]], $foreignKey, $relation, $subRelation, $closure, $cache);
            // 关联属性名
