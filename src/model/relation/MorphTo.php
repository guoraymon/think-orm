...
class MorphTo extends Relation
{
     ...
     */
    public function __construct(Model $parent, $morphType, $morphKey, array $alias = [], $relation = null)
    {
        $this->parent = $parent;
    ...
    public function eagerlyResultSet(array &$resultSet, $relation, array $subRelation, Closure $closure = null, array $cache = [])
    {
        ...
        if (!empty($range)) {
            ...
            foreach ($range as $key => $val) {
                ...
                $obj = new $model();
                $pk = $obj->getPk();
                $list = $obj->with($subRelation)->cache(isset($cache[0]) ? $cache[0] : false, isset($cache[1]) ? $cache[1] : null, isset($cache[2]) ? $cache[2] : null)->select($val);
                $data = [];
     ...
     * @return integer
     */
    public function relationCount(Model $result, Closure $closure = null, $aggregate = 'count', $field = '*')
    {
    }
    /**
     * 多态MorphTo 关联模型预查询
     ...
     * @return void
     */
    protected function eagerlyMorphToOne($model, $relation, Model $result, array $subRelation = [], array $cache = [])
    {
        // 预载入关联查询 支持嵌套预载入
        $pk = $this->parent->{$this->morphKey};
        $data = (new $model())->with($subRelation)->cache(isset($cache[0]) ? $cache[0] : false, isset($cache[1]) ? $cache[1] : null, isset($cache[2]) ? $cache[2] : null)->find($pk);
        if ($data) {
     ...
     * @return Model
     */
    public function associate(Model $model, $type = '')
    {
        $morphKey = $this->morphKey;
        $morphType = $this->morphType;
        $pk = $model->getPk();
