...
class MorphMany extends Relation
{
    ...
    public function eagerlyResult(Model $result, $relation, array $subRelation = [], Closure $closure = null, array $cache = [])
    {
        ...
        if (isset($result->{$pk})) {
            $key = $result->{$pk};
            $data = $this->eagerlyMorphToMany([[$this->morphKey, '=', $key], [$this->morphType, '=', $this->type]], $relation, $subRelation, $closure, $cache);
            if (!isset($data[$key])) {
                $data[$key] = [];
            }
    ...
    public function relationCount(Model $result, Closure $closure = null, $aggregate = 'count', $field = '*', &$name = null)
    {
        ...
        return $this->query->where([[$this->morphKey, '=', $result->{$pk}], [$this->morphType, '=', $this->type]])->{$aggregate}($field);
    }
    /**
     * 获取关联统计子查询
     * @access public
    ...
    public function getRelationCountQuery(Closure $closure = null, $aggregate = 'count', $field = '*', &$name = null)
    {
        ...
        return $this->query->whereExp($this->morphKey, '=' . $this->parent->getTable() . '.' . $this->parent->getPk())->where($this->morphType, '=', $this->type)->fetchSql()->{$aggregate}($field);
    }
    /**
     * 多态一对多 关联模型预查询
     * @access protected
     ...
     * @return array
     */
    protected function eagerlyMorphToMany(array $where, $relation, array $subRelation = [], Closure $closure = null, array $cache = [])
    {
        // 预载入关联查询 支持嵌套预载入
