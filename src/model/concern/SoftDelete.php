...
trait SoftDelete
{
    ...
    public function delete()
    {
        ...
        if ($name && !$this->isForce()) {
            ...
            $where = $this->getWhere();
            // 删除当前模型数据
            $result = $this->db()->where($where)->removeOption('soft_delete')->delete();
            $this->lazySave(false);
        }
        // 关联删除
    ...
    public function restore($where = [])
    {
        ...
        // 恢复删除
        $this->db(false)->where($where)->useSoftDelete($name, $this->getWithTrashedExp())->update([$name => $this->defaultSoftDelete]);
     ...
     * @return void
     */
    protected function withNoTrashed(Query $query)
    {
