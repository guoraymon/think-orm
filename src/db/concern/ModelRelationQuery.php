...
trait ModelRelationQuery
{
     ...
     * @return $this
     */
    protected function withAggregate($relations, $aggregate = 'count', $field = '*', $subQuery = true)
    {
        if (!$subQuery) {
     ...
     * @return $this
     */
    public function withCache($relation = true, $key = true, $expire = null, $tag = null)
    {
     ...
     * @return void
     */
    protected function resultToModel(array &$result, array $options = [], $resultSet = false, array $withRelationAttr = [])
    {
        // 动态获取器
        ...
        if (!$resultSet && !empty($options['with'])) {
            $result->eagerlyResult($result, $options['with'], $withRelationAttr, false, isset($options['with_cache']) ? $options['with_cache'] : false);
        }
        // JOIN预载入查询
        if (!$resultSet && !empty($options['with_join'])) {
            $result->eagerlyResult($result, $options['with_join'], $withRelationAttr, true, isset($options['with_cache']) ? $options['with_cache'] : false);
        }
        // 关联统计
