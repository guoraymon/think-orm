...
abstract class PDOConnection extends Connection
{
     ...
     * @var array
     */
    protected $bindType = ['string' => PDO::PARAM_STR, 'str' => PDO::PARAM_STR, 'integer' => PDO::PARAM_INT, 'int' => PDO::PARAM_INT, 'boolean' => PDO::PARAM_BOOL, 'bool' => PDO::PARAM_BOOL, 'float' => self::PARAM_FLOAT];
    /**
     * 服务器断线标识字符
     * @var array
     */
    protected $breakMatchStr = ['server has gone away', 'no connection to the server', 'Lost connection', 'is dead or not enabled', 'Error while sending', 'decryption failed or bad record mac', 'server closed the connection unexpectedly', 'SSL connection has been closed unexpectedly', 'Error writing data to the connection', 'Resource deadlock avoided', 'failed with errno'];
    /**
     * 绑定参数
     * @var array
     */
     ...
     * @return string
     */
    protected abstract function parseDsn(array $config);
     ...
     * @throws \Throwable
     */
    public function query(BaseQuery $query, $sql, array $bind = [])
    {
        // 分析查询表达式
        $query->parseOptions();
     ...
     * @throws \Throwable
     */
    public function execute(BaseQuery $query, $sql, array $bind = [], $origin = false)
    {
        if ($origin) {
            $query->parseOptions();
        }
     ...
     * @return mixed
     */
    public function value(BaseQuery $query, $field, $default = null, $one = true)
    {
        $options = $query->parseOptions();
     ...
     * @return mixed
     */
    public function aggregate(BaseQuery $query, $aggregate, $field, $force = false)
    {
        ...
        return $force ? (double)$result : $result;
    }
        ...
        foreach ($bind as $key => $val) {
            ...
            if (is_array($val)) {
                if (PDO::PARAM_INT == $val[1] && '' === $val[0]) {
                    ...
                    $val[0] = is_string($val[0]) ? (double)$val[0] : $val[0];
                    $val[1] = PDO::PARAM_STR;
                }
     ...
     * @return mixed
     */
    public function getLastInsID(BaseQuery $query, $sequence = null)
    {
     ...
     * @return mixed
     */
    protected function autoInsIDType(BaseQuery $query, $insertId)
    {
        $pk = $query->getPk();
        if (is_string($pk)) {
     ...
     * @return void
     */
    protected function triggerSql($sql, $runtime, $master = false)
    {
    ...
    protected function multiConnect($master = false)
    {
        ...
        if ($m != $r) {
            $dbMaster = [];
            foreach (['username', 'password', 'hostname', 'hostport', 'database', 'dsn', 'charset'] as $name) {
                $dbMaster[$name] = isset($config[$name][$m]) ? $config[$name][$m] : $config[$name][0];
            }
        }
        $dbConfig = [];
        foreach (['username', 'password', 'hostname', 'hostport', 'database', 'dsn', 'charset'] as $name) {
