...
class Mongo extends Connection
{
     ...
     * @throws RuntimeException
     */
    public function mongoQuery(BaseQuery $query, $mongoQuery)
    {
        $options = $query->parseOptions();
     ...
     * @return mixed
     */
    protected function getResult($typeMap = null)
    {
        // 设置结果数据类型
        if (is_null($typeMap)) {
     ...
     * @return array
     */
    public function column(BaseQuery $query, $field, $key = '')
    {
        $options = $query->parseOptions();
        if (isset($options['projection'])) {
            $query->removeOption('projection');
     ...
     * @return array
     */
    public function cmd(BaseQuery $query, $command, $extra = null, $db = '')
    {
