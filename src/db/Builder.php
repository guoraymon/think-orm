...
abstract class Builder
{
     ...
     * @var array
     */
    protected $parser = ['parseCompare' => ['=', '<>', '>', '>=', '<', '<='], 'parseLike' => ['LIKE', 'NOT LIKE'], 'parseBetween' => ['NOT BETWEEN', 'BETWEEN'], 'parseIn' => ['NOT IN', 'IN'], 'parseExp' => ['EXP'], 'parseNull' => ['NOT NULL', 'NULL'], 'parseBetweenTime' => ['BETWEEN TIME', 'NOT BETWEEN TIME'], 'parseTime' => ['< TIME', '> TIME', '<= TIME', '>= TIME'], 'parseExists' => ['NOT EXISTS', 'EXISTS'], 'parseColumn' => ['COLUMN']];
    /**
     * SELECT SQL表达式
     * @var string
     */
    ...
    protected function parseDataBind(Query $query, $key, $data, array $bind = [])
    {
        ...
        $name = $query->bindValue($data, isset($bind[$key]) ? $bind[$key] : PDO::PARAM_STR);
        return ':' . $name;
    }
    /**
     * 字段名分析
     ...
     * @return string
     */
    protected function parseExtra(Query $query, $extra)
    {
     ...
     * @return string
     */
    protected function parseClousreWhere(Query $query, Closure $value, $logic)
    {
    ...
    protected function parseWhereItem(Query $query, $field, array $val, array $binds = [])
    {
        ...
        foreach ($this->parser as $fun => $parse) {
            if (in_array($exp, $parse)) {
                return $this->{$fun}($query, $key, $exp, $value, $field, $bindType, isset($val[2]) ? $val[2] : 'AND');
            }
        }
     ...
     * @return string
     */
    protected function parseExp(Query $query, $key, $exp, Raw $value, $field, $bindType)
    {
        // 表达式查询
    ...
    protected function parseBetweenTime(Query $query, $key, $exp, $value, $field, $bindType)
    {
        ...
        return $key . ' ' . substr($exp, 0, -4) . $this->parseDateTime($query, $value[0], $field, $bindType) . ' AND ' . $this->parseDateTime($query, $value[1], $field, $bindType);
    }
    /**
     * IN查询
     * @access protected
     ...
     * @return string
     */
    protected function parseClosure(Query $query, Closure $call, $show = true)
    {
     ...
     * @return string
     */
    protected function parseDateTime(Query $query, $value, $key, $bindType)
    {
        $options = $query->getOptions();
        // 获取时间字段类型
        if (strpos($key, '.')) {
     ...
     * @return string
     */
    public function select(Query $query, $one = false)
    {
        $options = $query->getOptions();
        return str_replace(['%TABLE%', '%DISTINCT%', '%EXTRA%', '%FIELD%', '%JOIN%', '%WHERE%', '%GROUP%', '%HAVING%', '%ORDER%', '%LIMIT%', '%UNION%', '%LOCK%', '%COMMENT%', '%FORCE%'], [$this->parseTable($query, $options['table']), $this->parseDistinct($query, $options['distinct']), $this->parseExtra($query, $options['extra']), $this->parseField($query, $options['field']), $this->parseJoin($query, $options['join']), $this->parseWhere($query, $options['where']), $this->parseGroup($query, $options['group']), $this->parseHaving($query, $options['having']), $this->parseOrder($query, $options['order']), $this->parseLimit($query, $one ? '1' : $options['limit']), $this->parseUnion($query, $options['union']), $this->parseLock($query, $options['lock']), $this->parseComment($query, $options['comment']), $this->parseForce($query, $options['force'])], $this->selectSql);
    }
    /**
     * 生成Insert SQL
     * @access public
    ...
    public function insert(Query $query)
    {
        ...
        return str_replace(['%INSERT%', '%TABLE%', '%EXTRA%', '%FIELD%', '%DATA%', '%COMMENT%'], [!empty($options['replace']) ? 'REPLACE' : 'INSERT', $this->parseTable($query, $options['table']), $this->parseExtra($query, $options['extra']), implode(' , ', $fields), implode(' , ', $values), $this->parseComment($query, $options['comment'])], $this->insertSql);
    }
    /**
     * 生成insertall SQL
     * @access public
    ...
    public function insertAll(Query $query, array $dataSet)
    {
        ...
        return str_replace(['%INSERT%', '%TABLE%', '%EXTRA%', '%FIELD%', '%DATA%', '%COMMENT%'], [!empty($options['replace']) ? 'REPLACE' : 'INSERT', $this->parseTable($query, $options['table']), $this->parseExtra($query, $options['extra']), implode(' , ', $fields), implode(' UNION ALL ', $values), $this->parseComment($query, $options['comment'])], $this->insertAllSql);
    }
    /**
     * 生成slect insert SQL
     * @access public
    ...
    public function update(Query $query)
    {
        ...
        return str_replace(['%TABLE%', '%EXTRA%', '%SET%', '%JOIN%', '%WHERE%', '%ORDER%', '%LIMIT%', '%LOCK%', '%COMMENT%'], [$this->parseTable($query, $options['table']), $this->parseExtra($query, $options['extra']), implode(' , ', $set), $this->parseJoin($query, $options['join']), $this->parseWhere($query, $options['where']), $this->parseOrder($query, $options['order']), $this->parseLimit($query, $options['limit']), $this->parseLock($query, $options['lock']), $this->parseComment($query, $options['comment'])], $this->updateSql);
    }
    /**
     * 生成delete SQL
     * @access public
    ...
    {
        $options = $query->getOptions();
        return str_replace(['%TABLE%', '%EXTRA%', '%USING%', '%JOIN%', '%WHERE%', '%ORDER%', '%LIMIT%', '%LOCK%', '%COMMENT%'], [$this->parseTable($query, $options['table']), $this->parseExtra($query, $options['extra']), !empty($options['using']) ? ' USING ' . $this->parseTable($query, $options['using']) . ' ' : '', $this->parseJoin($query, $options['join']), $this->parseWhere($query, $options['where']), $this->parseOrder($query, $options['order']), $this->parseLimit($query, $options['limit']), $this->parseLock($query, $options['lock']), $this->parseComment($query, $options['comment'])], $this->deleteSql);
    }
}