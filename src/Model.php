...
abstract class Model implements JsonSerializable, ArrayAccess, Arrayable, Jsonable
{
     ...
     * @return void
     */
    protected function setUpdateWhere($where)
    {
     ...
     * @return $this
     */
    public function setQuery(Query $query, $clear = true)
    {
    ...
    public function db($scope = [])
    {
        ...
        if ($this->queryInstance) {
            ...
            $query = $this->db->connect($this->connection)->name($this->name . $this->suffix)->pk($this->pk);
        }
        if (!empty($this->table)) {
     ...
     * @return $this
     */
    public function refresh($relation = false)
    {
        if ($this->exists) {
    ...
    protected function updateData()
    {
        ...
        try {
            $where = $this->getWhere();
            $result = $db->where($where)->strict(false)->field($allowFields)->update($data);
     ...
     * @return bool
     */
    protected function insertData($sequence = null)
    {
        // 时间戳自动写入
        if ($this->autoWriteTimestamp) {
        ...
        $db->startTrans();
        try {
            $result = $db->strict(false)->field($allowFields)->replace($this->replace)->insert($this->data, false, $sequence);
            // 获取自动增长主键
     ...
     * @return static
     */
    public static function create(array $data, array $allowField = [], $replace = false)
    {
        $model = new static();
        if (!empty($allowField)) {
